...
{ "bg": "#RRGGBB", "ink": "#RRGGBB", "accent": "#RRGGBB", "headlineFont": "...", "bodyFont": "...", "notes": "..." }`;
      ...
      // ── generateDesignSpecPreview ──
      // A demo-sheet is a real reference IMAGE (via OpenAI's gpt-image-1) —
      // color swatches with hex labels, a headline specimen, a body-copy
      // specimen, and 2-3 example content mockups (post/carousel slide/
      // banner) built in that palette+type, plus suggested graphical/icon
      // treatment. This is a PREVIEW ONLY — nothing is written to Notion
      // here. "Approval" (per the operator) means persisting it, which is
      // approveDesignSpecPreview below.
      if (body.action === "generateDesignSpecPreview") {
        const { campaignId, productId, styleDirection } = body;
        if (!campaignId) return json({ error: "campaignId required" }, 400);
        if (!env.ANTHROPIC_API_KEY) return json({ error: "ANTHROPIC_API_KEY not configured" }, 500);
        if (!env.OPENAI_API_KEY) return json({ error: "OPENAI_API_KEY not configured — run: wrangler secret put OPENAI_API_KEY" }, 500);
        const hasProduct = productId && productId !== "__none__" && productId !== campaignId;

        const [researchRaw, campRaw, prodPage] = await Promise.all([
          fetch(`https://api.notion.com/v1/databases/${RESEARCH_DB}/query`, {
            method: "POST", headers: { ...dsHdr, "Content-Type": "application/json" },
        ...
        const campaignName = (cp.Name?.title || cp["Campaign Name"]?.title || []).map(t => t.plain_text).join("") || "Campaign";
        let productName = "";
        if (hasProduct && prodPage?.properties) productName = (prodPage.properties.Name?.title || []).map(t => t.plain_text).join("");

        const specPrompt = `${researchGuidelinesBlock(body.researchGuidelines)}You are a brand & visual designer. Propose ONE coherent design spec for this campaign${productName ? `'s product "${productName}"` : ""}'s social content.

CAMPAIGN: ${campaignName}
Keywords: ${rt(researchRaw, "Keywords")}
Statement: ${rt(researchRaw, "Statement")}
Unique Opportunity: ${rt(researchRaw, "Unique Opportunity")}
Pain Points: ${(cp["Pain Points"]?.rich_text || []).map(t => t.plain_text).join("")}
${styleDirection ? `OPERATOR STYLE DIRECTION (follow this): ${styleDirection}\n` : ""}
Give:
- name: 2-4 words naming the aesthetic
...
- headlineFont / bodyFont: real Google Fonts family names that fit the aesthetic.
- aesthetic: 2-3 sentences describing the visual direction, mood, imagery style, and what to avoid.

Return ONLY this JSON, no other text, no markdown fences:
{ "name": "...", "bg": "#RRGGBB", "ink": "#RRGGBB", "accent": "#RRGGBB", "headlineFont": "...", "bodyFont": "...", "aesthetic": "..." }`;

        const specResp = await fetch("https://api.anthropic.com/v1/messages", {
          method: "POST",
          headers: { "x-api-key": env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json" },
          body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 700, messages: [{ role: "user", content: specPrompt }] }),
        });
        const specData = await specResp.json();
        if (!specResp.ok) return json({ error: specData.error?.message || "Claude API error" }, 500);
        let spec;
        try {
          const raw = specData.content?.[0]?.text || "";
          const s = raw.indexOf("{"), e = raw.lastIndexOf("}");
          if (s === -1 || e === -1 || e < s) throw new Error("No JSON object found");
          spec = JSON.parse(sanitizeJsonControlChars(raw.slice(s, e + 1)));
        } catch(e) { return json({ error: "Failed to parse design spec JSON: " + e.message }, 500); }

        const demoSheetPrompt = `A brand style / design demo sheet, laid out like a designer's mood board on a single sheet. Aesthetic: ${spec.aesthetic || ""}.

Include, clearly arranged in a grid layout:
1. A row of labeled color swatches for these exact colors with their hex codes printed under each swatch: background ${spec.bg}, ink/text ${spec.ink}, accent ${spec.accent}.
2. A large headline type specimen — render the actual words "${campaignName}" in a typeface with the character and feel of "${spec.headlineFont}", at large display size, in the ink color on the background color.
3. A body-copy type specimen — 2-3 lines of realistic paragraph text in a typeface with the character and feel of "${spec.bodyFont}", smaller size, showing how it reads.
4. Three small example content mockups rendered in this exact palette and typography: (a) a square social media post, (b) one slide of a carousel, (c) a banner/header graphic — each with plausible sample headline+body text, not lorem ipsum.
5. A short row of suggested graphical elements / iconography in this style (simple icon shapes, dividers, or decorative marks that fit the aesthetic).

Clean, professional reference-sheet layout with clear section labels. No watermarks, no placeholder gray boxes.`;

        const imgResp = await fetch("https://api.openai.com/v1/images/generations", {
          method: "POST",
          headers: { "Authorization": `Bearer ${env.OPENAI_API_KEY}`, "Content-Type": "application/json" },
          body: JSON.stringify({ model: "gpt-image-1", prompt: demoSheetPrompt.slice(0, 4000), size: "1536x1024", quality: "high" }),
        });
        const imgData = await imgResp.json();
        if (!imgResp.ok) return json({ error: imgData.error?.message || "OpenAI image generation failed" }, 502);
        const b64 = imgData.data?.[0]?.b64_json;
        if (!b64) return json({ error: "OpenAI returned no image data" }, 502);

        return json({ spec, demoSheetImage: b64, demoSheetPrompt });
      }

      // ── approveDesignSpecPreview ──
      // "Approval" = persisting the previewed spec: creates the real Design
      // Spec record (same shape as createDesignSpec), uploads the demo sheet
      // image via Notion's file_uploads API, and attaches it to the campaign
      // (+ product, if one was selected) — same relation the campaign
      // "Design" section and every spec picker already read from.
      if (body.action === "approveDesignSpecPreview") {
        const { campaignId, productId, name, bg, ink, accent, headlineFont, bodyFont, aesthetic, demoSheetImage, demoSheetPrompt } = body;
        if (!name || !demoSheetImage) return json({ error: "name and demoSheetImage required" }, 400);
        const rtB = v => v ? [{ type: "text", text: { content: String(v).slice(0, 2000) } }] : [];
        const props = {
          Name: { title: [{ type: "text", text: { content: String(name).slice(0, 100) } }] },
          Background: { rich_text: rtB(bg) }, Ink: { rich_text: rtB(ink) }, Accent: { rich_text: rtB(accent) },
          "Headline Font": { rich_text: rtB(headlineFont) }, "Body Font": { rich_text: rtB(bodyFont) },
          "Aesthetic Description": { rich_text: rtB(aesthetic) },
          "Demo Sheet Prompt": { rich_text: rtB(demoSheetPrompt) },
        };
        if (campaignId) props["Campaigns"] = { relation: [{ id: dsDash(campaignId) }] };
        if (productId && productId !== "__none__" && productId !== campaignId) props["Products"] = { relation: [{ id: dsDash(productId) }] };
        const createResp = await fetch("https://api.notion.com/v1/pages", {
        ...
        const specId = created.id.replace(/-/g,"");

        // Upload the demo sheet image (already-generated base64 PNG) via the
        // same Notion file_uploads flow used for Login pictures/files.
        try {
          const binary = atob(demoSheetImage);
          const bytes = new Uint8Array(binary.length);
          for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
          const upInit = await fetch("https://api.notion.com/v1/file_uploads", {
            method: "POST", headers: { ...dsHdr, "Content-Type": "application/json" },
            body: JSON.stringify({ content_type: "image/png", mode: "single_part" }),
          });
          const upInitData = await upInit.json();
          if (upInit.ok && upInitData.id) {
            const formData = new FormData();
            formData.append("file", new Blob([bytes], { type: "image/png" }), "demo-sheet.png");
            const putResp = await fetch(upInitData.upload_url, { method: "POST", headers: dsHdr, body: formData });
            if (putResp.ok) {
              await fetch(`https://api.notion.com/v1/pages/${dsDash(specId)}`, {
                method: "PATCH", headers: { ...dsHdr, "Content-Type": "application/json" },
                body: JSON.stringify({ properties: { "Demo Sheet": { files: [{ type: "file_upload", name: "demo-sheet.png", file_upload: { id: upInitData.id } }] } } }),
              });
            }
          }
        } catch(e) { /* spec is still created/attached even if the image attach fails */ }

        return json({ success: true, id: specId, name });
      }
