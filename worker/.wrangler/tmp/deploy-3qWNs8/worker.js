...
var CAMPAIGNS_DB = "087b1163b4e64975bc7a4b686ff801de";
var CONTENT_STRATEGY_DB = "9fa5f42f010b47e7a82032607e07d6a1";
var PRODUCTS_DB = "e92fcfce75fc4f54b553df0b7672ff48";
var MAIN_TD_DB = "3471f7d3a4bb80de87c1d9e850f4a426";
var METHODS_DB = "285ed0b668be4dad89dfd090350096bc";
var LOGINS_DB = "72d262278a4c4786b375959432fdd82a";
var PLATFORMS_DB = "8248b700ebb7428aa28d8b5246509898";
var ASSETS_DB = "e91bdb6e770b4d298e9f62166a0fd5de";
var RESEARCH_DB = "557e6b7b8c434a578d45ecb0a8329f63";
var LEADS_DB = "e4518a459f004eb0b9646e48d8718705";
var SM_ACCOUNTS_DB = "aa6a16f2a77245bfb5efd9a8eb314b07";
var EMAILS_DB = "6252e9917027488fb628436aabb89947";
var SM_POSTS_DB = "addcfe1d1beb46dbbcaa397504a8041d";
var CORS = {
...
__name(stripMcpEscaping, "stripMcpEscaping");
...
__name(notionQuery, "notionQuery");
async function signToken(secret) {
  const payload = { exp: Date.now() + 8 * 3600 * 1e3, v: 1 };
...
var worker_default = {
  async fetch(request, env) {
    ...
    if (body.action === "pinUpdate") {
      ...
      const dash = /* @__PURE__ */ __name((i) => i.replace(/-/g, "").replace(/^(.{8})(.{4})(.{4})(.{4})(.{12})$/, "$1-$2-$3-$4-$5"), "dash");
      const props = {};
      if (voiceId !== void 0) props["Voice ID"] = { rich_text: [{ type: "text", text: { content: (voiceId || "").slice(0, 200) } }] };
      if (captionStyle !== void 0) props["Caption Style"] = { rich_text: [{ type: "text", text: { content: (captionStyle || "").slice(0, 2e3) } }] };
      if (voiceSettings !== void 0) props["Voice Settings"] = { rich_text: [{ type: "text", text: { content: (voiceSettings || "").slice(0, 2e3) } }] };
    ...
    if (body.action === "submitLead") {
      ...
      if (tsToken) {
        ...
        if (!tsData.success) return json({ error: "CAPTCHA verification failed \xE2\u20AC\u201D please try again" }, 403);
      }
      ...
      const dashId = /* @__PURE__ */ __name((raw) => {
        const s = raw.replace(/-/g, "");
        return s.slice(0, 8) + "-" + s.slice(8, 12) + "-" + s.slice(12, 16) + "-" + s.slice(16, 20) + "-" + s.slice(20);
      }, "dashId");
      const now = (/* @__PURE__ */ new Date()).toISOString();
      const name = "Lead \xE2\u20AC\u201D " + (campaign || "unknown") + " \xE2\u20AC\u201D " + now.slice(0, 16).replace("T", " ");
      ...
      const result = await resp.json();
      if (!resp.ok) return json({ error: "Submission failed \xE2\u20AC\u201D please try again" }, resp.status);
      return json({ success: true });
    }
    ...
    try {
      if (body.action === "getDevTitles") {
        ...
        titleRows.forEach((t) => {
          ...
          const camp = campById[campId] || { name: "\xCE\u201C\xC3\u2021\xC3\xB6", site: "Other" };
        ...
        const campaigns2 = Object.entries(campTitles).map(([campId, camp]) => {
        ...
        campaigns2.sort((a, b) => b.devCount - a.devCount);
      ...
      if (body.action === "deleteTdItem") {
        ...
        if (!resp.ok) {
          ...
          return json({ error: r.message || "Delete failed" }, resp.status);
        }
        return json({ success: true });
      }
      ...
      if (body.action === "createPlatform") {
        ...
        if (!title) return json({ error: "title required" }, 400);
        const platStatus = status || "Publish";
        const resp = await fetch("https://api.notion.com/v1/pages", {
          method: "POST",
          headers: { "Authorization": `Bearer ${NOTION_TOKEN}`, "Notion-Version": NOTION_VERSION, "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (body.action === "deleteTitle") {
        ...
        const dash = /* @__PURE__ */ __name((id) => id.replace(/-/g, "").replace(/^(.{8})(.{4})(.{4})(.{4})(.{12})$/, "$1-$2-$3-$4-$5"), "dash");
      ...
      if (body.action === "getTodos") {
        ...
        const todos = await Promise.all(results.map(async (page) => {
          ...
          const category = priorities.includes("daily content") ? "daily content" : priorities.includes("daily household") ? "daily household" : priorities.includes("get") ? "get" : "high";
      ...
      if (body.action === "getChildren") {
        ...
        const data = await resp.json();
        const pages = (data.results || []).filter((b) => b.type === "child_page").map((b) => ({ id: b.id.replace(/-/g, ""), name: b.child_page?.title || "Untitled", icon: "" }));
        return json({ pages });
      }
      ...
      if (body.action === "condense") {
        ...
        if (!text) return json({ html: "<p>\xCE\u201C\xC3\u2021\xC3\xB6</p>" });
...
FORMAT \xCE\u201C\xC3\u2021\xC3\xB6 each entry on its own line:
HEADING: body text
...
- Output only the entries, nothing else`,
      ...
      if (body.action === "getPublishedPosts") {
        ...
        const posts = await Promise.all(titleRows.map(async (page) => {
          ...
          try {
            ...
            if (textParts.join(" ").length > 280) excerpt += "\xE2\u20AC\xA6";
          } catch {
          }
      ...
      if (body.action === "updateLoginFull") {
        ...
        if (category !== void 0) props.Category = category ? { select: { name: category } } : { select: null };
        if (usr !== void 0) props.Usr = { rich_text: usr ? [{ type: "text", text: { content: usr } }] : [] };
        if (accountUrl !== void 0) props["Account URL"] = accountUrl ? { url: accountUrl } : { url: null };
        if (headline !== void 0) props.Headline = { rich_text: headline ? [{ type: "text", text: { content: headline } }] : [] };
        if (bio !== void 0) props.Bio = { rich_text: bio ? [{ type: "text", text: { content: bio } }] : [] };
        if (platformId !== void 0) props.Platform = platformId ? { relation: [{ id: dash(platformId) }] } : { relation: [] };
        if (smAccountIds !== void 0) props["SM Account"] = { relation: (smAccountIds || []).map((id) => ({ id: dash(id) })) };
        if (smAccountId !== void 0) props["SM Account ID"] = { rich_text: smAccountId ? [{ type: "text", text: { content: smAccountId } }] : [] };
      ...
      if (body.action === "linkLoginToCell") {
        ...
        const nodash = /* @__PURE__ */ __name((id) => id.replace(/-/g, ""), "nodash");
      ...
      if (body.action === "unlinkLoginFromCell") {
        ...
        const page = await pageResp.json();
        if (!pageResp.ok) return json({ error: page.message || "Fetch failed" }, pageResp.status);
        const campIds = (page.properties?.Campaign?.relation || []).map((r) => nodash(r.id)).filter((id) => id !== nodash(campaignId));
        const resp = await fetch(`https://api.notion.com/v1/pages/${dash(loginId)}`, {
          method: "PATCH",
          headers: { "Authorization": `Bearer ${NOTION_TOKEN}`, "Notion-Version": NOTION_VERSION, "Content-Type": "application/json" },
          body: JSON.stringify({ properties: {
      ...
      if (body.action === "getEmails") {
        ...
        const emails = rows.map((r) => {
          ...
          const txt = /* @__PURE__ */ __name((prop) => prop?.rich_text?.map((t) => t.plain_text).join("") || prop?.title?.map((t) => t.plain_text).join("") || "", "txt");
          return {
            id: r.id.replace(/-/g, ""),
            name: txt(p.Email),
            domain: txt(p.Domain)
      ...
      if (body.action === "getSmAccounts") {
        ...
        const accounts = rows.map((r) => {
          ...
          return {
            ...
            pw: txt(p.PW),
            emailIds: (p.Emai?.relation || []).map((r2) => r2.id.replace(/-/g, "")),
      ...
      if (body.action === "createSmAccount") {
        ...
        if (!name) return json({ error: "name required" }, 400);
        const dash = /* @__PURE__ */ __name((i) => {
          const s = i.replace(/-/g, "");
          return s.slice(0, 8) + "-" + s.slice(8, 12) + "-" + s.slice(12, 16) + "-" + s.slice(16, 20) + "-" + s.slice(20);
        }, "dash");
        const rt = /* @__PURE__ */ __name((v) => v ? [{ type: "text", text: { content: v } }] : [], "rt");
        const props = {
          Name: { title: [{ type: "text", text: { content: name } }] },
      ...
      if (body.action === "updateSmAccount") {
        ...
        const rt = /* @__PURE__ */ __name((v) => v != null ? [{ type: "text", text: { content: v } }] : [], "rt");
        const props = {};
        ...
        if (emailId !== void 0) props.Emai = emailId ? { relation: [{ id: dash(emailId) }] } : { relation: [] };
...
Rules:
- Hook in the first 2-3 seconds \xE2\u20AC\u201D grab attention immediately
- Write pure spoken voiceover text only \xE2\u20AC\u201D no brackets, no stage directions, no labels
- Target 75-100 words (30-45 seconds at ~2.3 words/second)
- End with a thought, question, or statement that lingers
${voiceGuide ? "- Honour the narrator voice guide above \xE2\u20AC\u201D pace, tone, register, and sentence length should match that delivery style" : "- Conversational rhythm, short sentences, natural pauses implied by punctuation"}
...
Output the script text only. No preamble, no labels.`;
        ...
        if (script) updateProps["Script"] = { rich_text: [{ type: "text", text: { content: script.slice(0, 2e3) } }] };
        const resp = await fetch(`https://api.notion.com/v1/pages/${dash(id)}`, {
          method: "PATCH",
          headers: { "Authorization": `Bearer ${NOTION_TOKEN}`, "Notion-Version": NOTION_VERSION, "Content-Type": "application/json" },
      ...
      if (body.action === "updateSmPostSettings") {
        ...
        if (voiceId !== void 0) props["Voice ID"] = { rich_text: [{ type: "text", text: { content: (voiceId || "").slice(0, 200) } }] };
        if (captionStyle !== void 0) props["Caption Style"] = { rich_text: [{ type: "text", text: { content: (captionStyle || "").slice(0, 2e3) } }] };
        if (backgroundImage !== void 0) props["Background Image"] = { rich_text: [{ type: "text", text: { content: (backgroundImage || "").slice(0, 500) } }] };
        if (voiceSettings !== void 0) props["Voice Settings"] = { rich_text: [{ type: "text", text: { content: (voiceSettings || "").slice(0, 2e3) } }] };
        if (body.imageStyleDna !== void 0) props["Image Style DNA"] = { rich_text: [{ type: "text", text: { content: (body.imageStyleDna || "").slice(0, 2e3) } }] };
        if (body.script !== void 0) props["Script"] = { rich_text: [{ type: "text", text: { content: (body.script || "").slice(0, 2e3) } }] };
        if (!Object.keys(props).length) return json({ success: true });
        const resp = await fetch(`https://api.notion.com/v1/pages/${dash(id)}`, {
          method: "PATCH",
          headers: { "Authorization": `Bearer ${NOTION_TOKEN}`, "Notion-Version": NOTION_VERSION, "Content-Type": "application/json" },
      ...
      if (body.action === "updateSmPostScript") {
        ...
        const resp = await fetch(`https://api.notion.com/v1/pages/${dash(id)}`, {
          method: "PATCH",
          headers: { "Authorization": `Bearer ${NOTION_TOKEN}`, "Notion-Version": NOTION_VERSION, "Content-Type": "application/json" },
          body: JSON.stringify({ properties: { "Script": { rich_text: [{ type: "text", text: { content: (script || "").slice(0, 2e3) } }] } } })
        });
        const result = await resp.json();
        if (!resp.ok) return json({ error: result.message || "Update failed" }, resp.status);
        return json({ success: true });
      ...
      if (body.action === "updateSmPostVideoPath") {
        ...
        const resp = await fetch(`https://api.notion.com/v1/pages/${dash(id)}`, {
          method: "PATCH",
          headers: { "Authorization": `Bearer ${NOTION_TOKEN}`, "Notion-Version": NOTION_VERSION, "Content-Type": "application/json" },
          body: JSON.stringify({ properties: { "Local Path": { rich_text: [{ type: "text", text: { content: (localPath || "").slice(0, 2e3) } }] } } })
        });
        const result = await resp.json();
        if (!resp.ok) return json({ error: result.message || "Update failed" }, resp.status);
        return json({ success: true });
      ...
      if (body.action === "runSmResearch") {
        ...
        const kws = Array.isArray(keywords) ? keywords.filter(Boolean) : keywords.split(",").map((s) => s.trim()).filter(Boolean);
        let tiktokItems = [];
        if (AT) {
          try {
        ...
        }).join("\n") || "(no TikTok data \xE2\u20AC\u201D proceed from keywords only)";
        ...
        }).join("\n") || "(no YouTube data \xE2\u20AC\u201D proceed from keywords only)";
        const fmtTTsummary = tiktokData.map(
          (v) => `- "${(v.text || v.desc || v.title || "").slice(0, 80)}" | views: ${v.playCount || v.stats?.playCount || 0}`
        ).join("\n") || "(no TikTok data)";
        const fmtYTsummary = ytData.map(
          (v) => `- "${(v.title || "").slice(0, 80)}" | views: ${v.viewCount || 0} | ${v.channelName || v.channelTitle || ""}`
        ).join("\n") || "(no YouTube data)";
        ...
        try {
          ...
          if (resRows.length) {
            await fetch(`https://api.notion.com/v1/pages/${resRows[0].id}`, {
              method: "PATCH",
              headers: { "Authorization": `Bearer ${NOTION_TOKEN}`, "Notion-Version": NOTION_VERSION, "Content-Type": "application/json" },
              body: JSON.stringify({ properties: { "TikTok Trends": { rich_text: [{ type: "text", text: { content: rawSummary.slice(0, 2e3) } }] } } })
            });
          }
        } catch (e) {
        }
...
TRENDING TIKTOK (indexed \xE2\u20AC\u201D copy URLs exactly for TopVideos):
${fmtTT}

TRENDING YOUTUBE (indexed \xE2\u20AC\u201D copy URLs exactly for TopVideos):
${fmtYT}
...
RULES: TopVideos must be real URLs copied exactly from the indexed lists. Pick the 3 that best match each specific idea's angle. If fewer than 3 URLs exist, use what is available. If no URLs at all, write "none".`;
        const cRes = await fetch("https://api.anthropic.com/v1/messages", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-api-key": (env.ANTHROPIC_API_KEY || "").trim(), "anthropic-version": "2023-06-01" },
          body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 2e3, messages: [{ role: "user", content: claudePrompt }] })
        });
        const cData = await cRes.json();
        ...
        for (const block of blocks.slice(0, 5)) {
          ...
          const script = (scriptM?.[1] || "").trim() || block.replace(/^IDEA\s+\d+\s*/i, "").replace(/Title:[^\n]+\n?/i, "").replace(/Platform:[^\n]+\n?/i, "").replace(/Script:\s*/i, "").replace(/TopVideos:[^\n]+\n?/i, "").trim();
          const topVidsRaw = (topVidM?.[1] || "").trim();
          const topVideos = topVidsRaw === "none" ? "" : topVidsRaw.split(/\s+/).filter((u) => u.startsWith("http")).slice(0, 3).join("\n");
          const platforms = [];
          ...
          const props = {
            ...
            "Post Copy": { rich_text: [{ type: "text", text: { content: script.slice(0, 2e3) } }] }
          };
      ...
      if (body.action === "saveTrade") {
        ...
        const trade = {
          ...
          entry_time: now,
          entry_price: null,
          // underlying at entry â€” filled by poller
          price_captured: false,
          current_price: null,
          current_pct: null,
          // % move of underlying since entry
          max_high: null,
          max_high_time: null,
          max_low: null,
          max_low_time: null,
