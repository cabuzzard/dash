...
3. **Content** — edit the `HUB` object in the bottom `<script>` (marked
   ...
   - `journal.items` — the **static fallback** for the blog section. When
     `campaignId` is set, `journal` is **live-fed**: `getHubBlog` reads this
     hub's own `web/hub/{slug}/blog/posts.json` (written by
     `publishSeoPostToLiveSite` when a `SEO Post` / `Blog - SEO - News` asset
     publishes for this campaign — see `docs/methods-titles-assets.md`) into
     `{kicker, title, excerpt, url}` cards linking `./blog/{slug}/`, plus an
     "All posts →" link to `./blog/`. Leave `items: []` and write a good
     `empty` state until posts exist. The blog pages themselves
     (`blog/{slug}/index.html`, `blog/index.html`) are generated by the worker,
     styled from this hub's `hubs.design.json` tokens/fonts — nothing to
     hand-author.
