...
A dashboard-driven generation also writes a full **Carousel Asset Package** after Hashtags — several optional headings, in order: `Content Summary` (core message / audience / funnel stage / primary goal), `Hook Package` (3 alternate opening hooks — Primary/Alt 1/Alt 2, useful if you want to swap the lead angle), `Slide Production Notes` (one dense bullet per slide: role, objective, key takeaway, layout/density/complexity notes, and `needs:` visual-support flags like `needsCustomVisual`/`needsDataViz`/`needsIconOrGraphic`/`needsPhotoOrIllustration` — flags only, never an illustration prompt or style description, since visual decisions belong to whoever builds the Canva design, i.e. you, in Step 3), `Content Flow` (how the 7 slides connect), `Educational Assets` (the teaching takeaways), `SEO / Publishing` (LinkedIn caption / X post / description / keywords / alt text / search intent — platform variants beyond the core IG caption/hashtags), and `Production Checklist` (pre-publish checks). Read whatever is present and let it inform the Canva build in Step 3 — the `needs:` flags on each slide are your cue for which slides want a custom visual, data viz, icon, or photo/illustration treatment vs. a pure-typographic one. Never treat the absence of any of these headings as missing anything; it's all optional metadata layered on top of the required Slide-N/Caption/Hashtags structure, not required structure itself. (Older titles may instead have a legacy `## Visual Plan` heading — one bullet per slide: purpose / visual objective / illustration direction — treat it the same way, as optional context.)
