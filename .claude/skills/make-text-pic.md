...
- Read the campaign's **Design Spec**: POST worker `getCampaignDesignSpecs { campaignId }`. Pick `designSpecId` if given, else the first spec whose `canvaLink` contains `/d/` (a real Canva-backed one), else the first spec, else fall back to a clean default. Keep its `bg / ink / accent / headlineFont / bodyFont / notes`.
