...
**Check the Asset's `Visual Production Brief` property first, before any of the above.** This is the ChatGPT-completed Visual Director output the operator pasted into the dashboard's Assemble modal (or directly into Notion) — if it's set, it OUTRANKS your own background-sourcing judgment entirely: read its per-scene approved treatments (illustration/diagram/icon/screenshot/existing asset/text-only — whichever the Visual Director actually chose) and its Machine-Readable Status block (`scene_01:` / `scene_02:` / ... entries with `treatment`/`status`/`asset`). For any scene marked `approved` or `ready_for_assembly`, look for an uploaded reference image at `web/{deployPath}/assembled/{assetSlug}/scene-NN.*` (the dashboard's "🖼️ Upload Images" modal hosts them there — deployPath/assetSlug follow the same slugified-name convention used throughout this repo, so check GitHub directly if the exact path isn't obvious) and use that as the real background for that beat instead of inventing or sourcing your own. If no `Visual Production Brief` is set, fall back to Design Notes/Images and Step 2's own sourcing as before — this is additive, not required.
