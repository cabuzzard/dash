...
Also read the Asset page's **"Shot / B-Roll Notes"** section alongside the script. A dashboard-generated script (the "Generate Remotion Video Asset Package" format) writes one dense bullet per scene here — scene role/objective/key takeaway, delivery intent, estimated duration, a compact production line (scene type, layout, density, composition, camera motion, transition, audio notes), a flow line (transition into next scene / continuity / retention reason / viewer expectation), and a `needs:` list of visual-support flags (`needsIllustration`, `needsMotionGraphics`, `textOnly`, `needsBRoll`, `needsWorkflowDiagram`, `needsScreenshot`, `needsIconAnimation`, `reuseExistingAsset`, `existingAssetCandidate`, `reusableVisualCandidate`). **This is planning metadata, not a ready-made image prompt** — by design, the generation step deliberately doesn't invent artwork or illustration-style descriptions (that's this skill's job, acting as the Visual Director for the one background image this render needs). Use the `needs:` flags and production/composition notes to inform your own background-sourcing judgment in Step 2, and let the motion/timing info in the flow/production lines inform the Remotion Ken Burns config. Older or manually-written scripts may only have a plain one-liner per beat, or nothing at all — that's fine, just less to work with; the rendering workflow itself doesn't change either way (still one background image + Ken Burns + word captions, not a multi-image sequence).
