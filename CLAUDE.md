...
Stage 1 (Visual Design Card) instructions were reworded to match: ChatGPT is told explicitly that every value it's receiving is already a resolved decision, not a blank — its job is to present it compactly, then edit specific values the operator flags, preserving everything else, never regenerating from a blank page. Stage 2 still requires every Required Variable Set entry to come back resolved, split across the Visual Brief (creative fields) and Complete Assembly Package (production fields) — none may be silently dropped.
