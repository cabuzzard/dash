...
**Gotcha that cost real debugging time (2026-08-27): a new Notion database is NOT automatically visible to the Worker.** After building the whole Post Type system, every slot's Post Type silently stayed empty — `generateGrowthStrategy`'s AI output was correct (`postType: "Pillar"` etc., verified via a direct authenticated call + `wrangler tail`), but `resolvePostTypeId` always returned null because `notionQuery(POST_TYPES_DB, {})` was returning `[]`. The `.catch(() => [])` on that query was swallowing the real error: `"Could not find database with ID: ... Make sure the relevant pages and databases are shared with your integration 'hermbuzz'."` Every other DB the Worker touches was already shared before this session; `POST_TYPES_DB` was created fresh via the Notion MCP connector (a *different* integration from the Worker's own `NOTION_TOKEN`/"hermbuzz"), and new databases don't inherit a parent page's integration access automatically. **Fix for next time a new database gets created for the Worker to use: share it with "hermbuzz" via Notion's `···` → Connections menu immediately, before wiring any code to it** — and prefer a logged catch (`.catch(e => { console.error(...); return []; })`) over a silent one on any `notionQuery` a new feature depends on, so this class of failure surfaces in `wrangler tail` in seconds instead of requiring a live-request debugging session.
