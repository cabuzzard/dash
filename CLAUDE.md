...
**Image plate spec (2026-09-10):** the Content Hubs card's Design section shows a **copyable "Image plate spec" field** — the full wordless-plate art-direction brief, assembled server-side (**`getImageBrief`** → `assembleImageBrief` + `writeImageSpec`) from the hub design record **+ campaign Research + the main product's 🔬 Product Research + operator guidance**. Fetched per hub, client-cached per session; **↻ Regenerate** forces a fresh assembly; **✎ Guidance** → `saveImageGuidance` writes Research `Image Direction` (folded into the spec and every render, overriding derived choices). Optional `hubs[slug].design.photography` (2-3 sentences) further sharpens it — care-gap has one. `design.photography` is also read by `generateOfferImage` and `scripts/grok-image.py --hub`; the ChatGPT design-brief prompt asks for a `photography:` line. Same spec `generateOfferImage` renders the offer plates against.
