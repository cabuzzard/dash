...
**Image plate spec (2026-09-10):** the Content Hubs card's Design section shows a **derived, always-current copyable field** (`buildHubImageSpec` in `index.html`) — a full "wordless plate" brief composed live from that hub's `hubs.design.json` record (palette hexes + notes, temperature-detected light, register, `design.avoided` → Never list, composition safe-areas, prompt skeleton + filled examples). Optional new key `hubs[slug].design.photography` (2-3 sentences of real-image direction) sharpens it — care-gap has one; without it the spec still generates with bracketed scene slots. The same `design.photography` is read by `generateOfferImage` (worker) and `scripts/grok-image.py --hub`. The ChatGPT design-brief prompt now asks for a `photography:` line.
