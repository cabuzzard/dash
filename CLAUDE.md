...
| 🔑 Keyword Clusters (SEO) | `28ece99103064fc6874aa45cd17b81e6` |
...
- Asset → Method attribution (2026-09-10) — **method is stamped on the Asset**, not the (method-agnostic) title. New `Assets.Method` relation; `assetMethodProp(methodId, assetType)` (`worker.js`, module scope) resolves it — explicit `methodId` else Asset Type → Method Name via `ASSET_TYPE_METHOD_ALIAS` — and every `ASSETS_DB` create spreads it in (all `generateTitleAssets` branches incl. **Content Hub** which previously stamped nothing, the skill actions, `runHubEmailSequence`, `createAsset`, `duplicateAsset`, `createAssetComponent`). `getHubMethodMatrix` now counts **assets** (Development / Publish / Published) by `asset.Method` (title.method = legacy fallback). `backfillAssetMethods` (re-runnable) fills existing assets. Fixes the multifamily-acquisitions Content Hub + Landing Page assets missing from the matrix.
- Single Post method (2026-09-10) — **renamed from `quote card`**; the old `Hook Posts` method + its Grok-plate/HTML-overlay render are archived. Fills one of a **hub's fixed Canva templates** with researched copy — one Asset per iteration, all on the same template for a run (`Asset Type: single post`). `generateTitleAssets` `/\bsingle post\b/i` branch: pick a `templateId` + `contentType` (Hook / Pain Point / Benefit / Informational / Customer Focus / Proof / Objection / Contrarian) + `count`; a forced `submit_single_posts` call writes N posts of `headlinePrimary` + `headlineAccent` + `body` + caption + hashtags + altText; each Asset gets a `SINGLE POST` json block + `Canva Template` stamped, Status Development, **no worker-side render**. Per-hub template registry in KV (`singlepost:templates:<slug>`, tagged to a method) — `getSinglePostConfig` / `buildSinglePostTemplatePrompt` (from the hub Image Spec) / `saveSinglePostTemplate` / `deleteSinglePostTemplate`, managed from the Content Hubs card's **Single-Post Templates** panel + New Template modal. **Stage 2:** `buildSinglePostCanvaHandoff {titleId}` → `.claude/skills/single-post-canva.md` → a Canva-connected Claude session `copy-design`s the template, fills it, `export-design`s a PNG → `saveSinglePostImage` (mirrors `saveOfferImage`: hosts on `Post Image`, sets `Design Link`, Status → Publish). Full spec: `docs/methods-titles-assets.md` § "Single Post".
...
- Intended Method tag on Development titles (2026-09-10) — a **method picker** on the bulk "Add Development Titles" modal (`index.html` `titleModal`) and the microsite's "Set Strategy" modal (`titleStrategyModal`), both writing the title's existing (method-agnostic) `method` relation as a pure tag — it doesn't affect title content. `createDevTitles` now accepts `methodId`; `updateTitleStrategy` now accepts `methodId` alongside `growthStrategyId`/`productId`, same send-only-if-changed contract. `getHubMethodMatrix`'s **dev** count switched from asset-status-Development counts to **Development-status TITLES tagged with the method** (hub via title Campaign, landing page via title product) — pub/pubd stay asset-driven (Publish/Published only).
- Carousel method consolidation (2026-09-11) — per operator direction ("I want one method called carousel for now"), the "carousel — Benefits Carousel LTE Template CSV Export" method was **renamed to `Carousel`** and is now the one production carousel method; "…BRN…", "…Template CSV Export", "…evaluate List Carousel…", "…Growth evaluate" are all superseded, kept in Development as raw material (never deleted). Its `generateTitleAssets` branch (the generic `/template csv/i` path, now also matched by exact name `/^carousel$/i` since the short name no longer contains "template csv") gained a **per-hub Canva template registry + picker** (KV `carousel:templates:<slug>`, same shape as single post's — `getCarouselConfig`/`saveCarouselTemplate`/`deleteCarouselTemplate`), simpler than single post's (no content-type/iterations — one asset per run): an explicit `templateId` wins, else the hub's first registered template, else the legacy fallback (the Method's own single `Template` property — unaffected for non-hub campaigns, which have no per-hub registry). Generate Assets modal: **Template** `<select>` + **+ new template** (name + Canva link only, no AI-generated background) shown when the method name is exactly "Carousel". `ASSET_TYPE_METHOD_ALIAS["carousel"]` updated to `"Carousel"`. Frontend gotcha: `gaMatchSkill` now explicitly excludes exact-name "Carousel" so it doesn't fall into the *other*, unrelated `/carousel(?!.*template csv)/i` → `generateCarouselPreview` real-image-rendering entry in `GA_SKILL_METHODS` (that regex's exclusion relied on the old long name always containing "template csv").
- SEO audit — Phase 1 of a bigger keyword-cluster/silo system (2026-09-11) — `extractSeoFacts(html, url)` uses Cloudflare's native **HTMLRewriter** to crawl a hub/landing page's LIVE deployed HTML (title/meta/OG/canonical, H1s/H2s, image alt coverage, internal/external links, word count) — grounded, not simulated. **Distinguishes "no H1" from "H1 present but empty at parse time"** (`h1Count` vs `h1s.length`): several hub pages render their hero/nav via client-side JS from an empty markup shell, which a non-JS crawler never sees — a materially different, worse finding than a missing tag, confirmed live against the surf-vacations hub during build. `runSeoAudit {campaignId}` also pulls the campaign's Research `Keywords` + every Publish/Published asset under it, grades everything against a **fixed, hand-maintained `SEO_AUDIT_CHECKLIST`** (not live-web-search-grounded — refined incrementally per operator direction) via one forced Claude tool call. Staged in KV (`seoaudit:<campaignId>`); `getSeoAudit`/`commitSeoAudit` freeze it onto the Research record's new "SEO Audit" field, same stage→commit shape as `saveImageSpec`. Content Hubs card: **🔍 SEO Audit** section (↻ Run audit / findings + action items / 💾 Post to brief). Next slice: the keyword-cluster/silo builder + product recommendations (P2-P4 in the operator's own phasing, tracked in the "Method pipeline grid" artifact).
- Method explainer (2026-09-11) — a "❓ How this works" box in the method-edit modal (opened by clicking a Hub Method Matrix column header): a plain-prose explanation of what the method produces and how, distinct from the raw Notion Notes/body shown right above it in the same modal. `generateMethodExplainerText(env, methodId)` (one Claude call, explicit instruction against jargon/headers/bullets) → cached in KV (`methodexplainer:<id>`) via `getMethodExplainer` (generate-once, cheap thereafter); `regenerateMethodExplainer` forces a fresh pass when the operator has since edited the method's own body.
- SEO keyword clusters/silos (2026-09-11, continuing the SEO Audit system) — new **🔑 Keyword Clusters** Notion DB (`SEO_KEYWORD_CLUSTERS_DB`, `28ece99103064fc6874aa45cd17b81e6`: Name/Cluster Keywords/Rationale/Status/Campaign) — deliberately distinct from the pre-existing `KEYWORD_CLUSTERS_DB` constant, which is the unrelated standalone Knowledge Atlas pipeline. `generateKeywordClusters {campaignId, guidance?}` groups the campaign's Research Keywords into 3-8 thematic clusters (one forced Claude tool call), aware of already-committed clusters (a re-run covers what's left) and existing product names (context only — **never matched against**, per operator: "recommended is ideal and does not pass over existing products"). Staged in KV (`seoclusters:staged:<campaignId>`); `updateStagedKeywordCluster` edits a candidate in place; `commitKeywordCluster` writes the real Notion row; `archiveKeywordCluster` for cleanup. Live-tested end to end on surf-vacations (generate → edit → commit → read-back all confirmed working, including the new DB's Connections sharing). Hierarchy note for the next slice (product creation from a cluster): a cluster's Name is meant to become its member products' existing free-text **Product Stack** label (reusing that mechanism app-wide, not inventing a parallel one). **Superseded by the next bullet** — this UI moved out of the Content Hubs card into the microsite.
- Keyword Clusters + Cluster Ideas move to the microsite; hub card keeps only On-page SEO (2026-09-11) — per operator direction, keyword-cluster staging doesn't belong on the dashboard's Content Hubs card; it's now built into every campaign's admin microsite (`microsites/hard-grind/index.html`, the sync source — `python microsites/sync_microsites.py` propagates), Keywords & Notes tab. That tab's Keywords column is relabeled **Main Keywords** (the one keyword set the campaign actually uses — On-page SEO Audit still reads this same Research `Keywords` field, unchanged) with a new **SEO Clusters** subsection below it: committed clusters + staged candidates, each its own collapsed group (purple = committed, amber = staged — same collapsed-by-default convention as Products' Product Stack groups, `COLLAPSED_KW_GROUPS`/`SEEN_KW_GROUPS`, session-only). **Naming convention:** a cluster's name is its single top/most-representative keyword from the pool, exactly as it appears there — not an invented phrase (`generateKeywordClusters` and the new `regenerateKeywordCluster` prompts both enforce this). New **`regenerateKeywordCluster {campaignId, clusterId, guidance?}`** regenerates exactly ONE staged cluster (every other staged/committed cluster's keywords passed as fixed/already-claimed, so a per-cluster ↻ regen button doesn't clobber siblings) — the bulk `generateKeywordClusters` action still covers the initial batch. New **`removeStagedKeywordCluster`** discards one staged idea outright (no Notion write, nothing was ever created). The Product Ideas column gained a parallel **Cluster Ideas** subsection: one collapsed group per staged cluster (named after that cluster), each with its own **`generateClusterProductStacks {campaignId, clusterId, guidance?}`** ("↻ propose products") — product concepts staged inline on the cluster object's own `products` array, invented fresh from that cluster's keywords, **never matched against existing products and never committed to the real Products DB** (pure ideation, per operator: "none of them are committed they are just ideas"); `removeClusterProductStack` dismisses one proposal. The pre-existing flat, Research-field-backed Product Ideas search (`getProductIdeas`/`runProductIdeas`/`r-prodideas`) is untouched and still there above this new subsection. Hub card's SEO details section is renamed **On-page SEO** and now holds only the 🔍 SEO Audit block — no keyword-cluster UI there anymore; its copy now says explicitly that it grades the page against "the campaign's committed keywords," i.e. Main Keywords set in the microsite.
- Title Status is independent of Asset Status (2026-09-11 fix) — a dozen+ `generateTitleAssets` branches (carousel, avatar video, text video/reel, explainer, offer, resume/job, t-shirt, news-blog rebuild, SEO pillar post) used to flip the **source title's own `Status` to "Publish"** the instant its first asset was created — a carryover from before titles became stage-driven ([[feedback_dash_dev_section_stage_driven]]). All of those writes are removed; a title's Status is now exclusively the operator's call (⇄ button / Set Strategy / stage promotions), never touched by asset generation, even though an asset relates back to its title. One-time (re-runnable) data fix: **`revertPublishedTitlesToDevelopment`** action — reverts every Content Strategy title still at Status "Publish" back to "Development", rate-limited/resumable same as `backfillAssetMethods`, never touches Assets. Triggered from the Hub Method Matrix's **⟳ revert titles** button (`hamRevertPublishedTitles`).
...
**Privacy policy (2026-09-24):** every hub has `<slug>/privacy/index.html`, **generated** by `node scripts/build-hubs.mjs` from `web/hub/privacy-template.html` (tokens/fonts/brand/email from `hubs.design.json`, domain from `_worker.js`; hubs with no `email` fall back to `info@webguyleadgeneration.com` with a warning). Edit the template, never the generated copies. Each hub footer carries a **static** `<a href="privacy/">Privacy policy</a>` (static on purpose — Google's brand-verification crawler doesn't run the JS that fills `footLegal`). Added so `aisystemimplementation.com` can serve as the brand-verified home page for the `keyword-research-509700` Google Cloud project (Google Ads API Basic access).
