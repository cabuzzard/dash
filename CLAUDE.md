...
2. **Stage 2 — two final markdown documents**, returned together once the Design Card is approved: the **Visual Brief (final)** — same per-slide/scene structure as the Design Card, but as settled fact reflecting the in-chat critique, not a proposal — and the **Complete Assembly Package** — one exhaustive, zero-ambiguity document via an explicit per-asset-type field checklist (`assemblyPackageChecklist` in `generateVisualBriefPrompt`): for Carousel, per-slide Layout Template/exact colors+fonts/verbatim copy placement/image-icon-diagram production instruction/alt text/safe-area+contrast compliance, plus the Platform & Export Preset and Carousel Design System values actually used; for Text Video, the Production Path declaration repeated first, per-scene visual/motion/color/timing spec, the mandatory render sequence verbatim if Custom Remotion Build, and voice/renderer requirements. No field may say "TBD" — every value is either concrete or explicitly marked DERIVED AT PRODUCTION. The instructions state explicitly that together these two documents must cover every variable needed to assemble the asset — a gap in either one is a defect, not an acceptable omission.

The **Visual Brief (final)** is what the operator downloads/pastes and uploads via 📄 Brief into the `Visual Production Brief` property; the **Complete Assembly Package** uploads via 🧱 Package into `Production Assembly Package`. The Design Card is intentionally never uploaded anywhere — it's a same-conversation critique step only. All three stage names are ChatGPT-facing labels; the underlying Notion property names are unchanged, so `uploadAssetDocument`/`assembleAsset` need no changes.

**`# Required Variable Set`** — a new section printed right after Scope, before Editing Authority, per operator request to spell out up front (in the same initial handoff, not buried at the end) the complete taxonomy of variables Stage 2 must return. 16 numbered categories (0 Asset Identity through 15 Deliverables — Visual Intent, Design System Resolution, Typography, Grid, Component Library, Iconography, Diagram/Illustration Language, per-slide/scene Layout System, Visual Continuity, Slide/Scene Specifications, Asset Manifest, QA Checklist, Production Manifest), built in `generateVisualBriefPrompt` from `rv()`-formatted lines. Where this system already has a value (`resolvedSpec`, `specDraft.spec`/`.scenes` for text video, `specDraft.ds`/`.slides` for carousel, Asset Metadata fields) it's shown inline as "our version"; everywhere else it's marked `Not yet specified by this system — propose and resolve one` rather than inventing false precision — most of Component Library, Asset Manifest, QA Checklist, and Production Manifest fall in this bucket today, since nothing in this system tracks per-component size/padding/corner-radius, layer-by-layer manifests, or a formal QA checklist yet. Stage 2 instructions require every one of these variables to come back resolved, split across the Visual Brief (creative fields) and Complete Assembly Package (production fields) — none may be silently dropped.
