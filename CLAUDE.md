...
**🏷️ Post Types — standalone global taxonomy (2026-08-27), distinct from the free-text `Type`/`Recurrence` above.** A new database (`POST_TYPES_DB`, Name + Rationale) holds a curated, operator-grown list of strategic content roles — seeded from Content Strategy's pre-existing `Post Type` select (Pillar/Sequential/Recurring/One-off/sm post), which is *not* the same axis as Strategy Slots' AI-improvised `Type` field (Teach/Take/Story/...) documented above; both now coexist. Strategy Slots gained two new relation properties: `Post Type` (→ `POST_TYPES_DB`) and `Platforms` (→ the existing `PLATFORMS_DB`, distinct from the older free-text `Platform`). `generateGrowthStrategy` best-effort matches its AI-generated `type`/`recommendedPlatform` strings against these catalogs by exact name and sets the relations when found — unmatched is fine, fixable from the UI. The microsite's Strategies panel titles each slot row by its Post Type (with Rationale as a native tooltip) instead of the slot's own Name, so a grouping's slots read as the strategy's *arc* at a glance (e.g. Pillar → Sequential → Sequential → Recurring). A **✏️ Edit Slot** button on every slot row opens a modal (`editSlotModal`) to change the Proposed Title (Angle), Post Type, or Best Platform(s) directly — each picker supports typing to search an existing entry or creating a new one inline (`createPostType`/`createPlatform`, mirroring the Login platform picker's autocomplete+create convention), so an operator never has to leave the microsite to grow either catalog. New worker actions: `getPostTypes`, `createPostType`, `updateStrategySlot` (patches Angle/Post Type/Platforms on one slot; `getPlatforms`/`createPlatform` already existed). Deliberately independent of Method for now, per operator direction — no Post Type → Method mapping exists yet.
