...
Every Visual Brief opens with a **`# Scope`** section, before Editing Authority, added after ChatGPT repeatedly responded to these documents with process critiques and architecture-redesign proposals instead of the requested output (the document's own rich framing — tiers, authority, systems — reads as an invitation to that). It's a blunt directive: this is a finished internal handoff for execution, not review; do not propose changes to the document, the system, or the workflow that produced it; every response in the conversation should be one of the two deliverables below. The closing `# Visual Director Instructions` repeats it as a second anchor late in a long back-and-forth. This is a prompt-engineering mitigation, not a guarantee — ChatGPT can still ignore it.

**Two-deliverable output contract.** `# Visual Director Instructions` requires exactly two deliverables, in order, both fully self-contained in this one document (no assumed prior conversation or dashboard knowledge — the whole point is it drops into a brand-new ChatGPT chat and still works):
1. **Visual Design Card** — a compact, critiquable proposal, one entry per slide/scene (visual concept in one sentence, resolved layout/palette/typography with a kept-or-changed-and-why note, illustration/icon/diagram style, composition notes, any concern) plus a top-level summary. ChatGPT stops here and waits for operator feedback — it does not proceed to Deliverable 2 until the direction is explicitly approved. This is what the operator uploads via 📄 Brief into the `Visual Production Brief` property.
2. **Complete Assembly Package** — one exhaustive, zero-ambiguity document, via an explicit per-asset-type field checklist (`assemblyPackageChecklist` in `generateVisualBriefPrompt`): for Carousel, per-slide Layout Template/exact colors+fonts/verbatim copy placement/image-icon-diagram production instruction/alt text/safe-area+contrast compliance, plus the Platform & Export Preset and Carousel Design System values actually used; for Text Video, the Production Path declaration repeated first, per-scene visual/motion/color/timing spec, the mandatory render sequence verbatim if Custom Remotion Build, and voice/renderer requirements. No field may say "TBD" — every value is either concrete or explicitly marked DERIVED AT PRODUCTION. This is what gets uploaded via 🧱 Package into `Production Assembly Package`. Both deliverable names are ChatGPT-facing labels only — the underlying Notion property names (`Visual Production Brief` / `Production Assembly Package`) are unchanged, so `uploadAssetDocument`/`assembleAsset` need no changes.
