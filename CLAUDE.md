...
**Not yet done, either type:** wiring into `assembleAsset` (which just requires both uploaded documents present, no per-field gating); syncing ChatGPT's revisions back into these databases (the operator applies them to the Notion rows by hand).
