...
**Not yet done:** the 147 pre-existing Strategy Slots (created before this rework) still have no Post Type set — they need the same research-grounded, per-slot AI assignment `generateGrowthStrategy` now does for new ones, not a mechanical heuristic. A first attempt at a heuristic backfill (Sequence→Sequential, Recurrence→Recurring) was started and immediately reverted once the corrected content-descriptor model above was clarified — don't repeat that mistake.
