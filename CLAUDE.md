...
`🚀 Growth Strategy` (`437b8c2615234b6bbe4a694b31f3000f`) — one record per generation run, related to a Product and Campaign. **Distinct from two other same-sounding things**: 🔬 Product Research (`PRODUCT_RESEARCH_DB`, the 11-field positioning doc, "who are we talking to and why" — see above) and Content Strategy (`CONTENT_STRATEGY_DB`, which is actually the Titles DB, confusingly). Growth Strategy answers "given that positioning, what should we actually make" — it recommends thematic title groupings, each with specific title angles, a Method (grounded in the product's actual linked Methods, never invented), and a platform.

Generated via `generateGrowthStrategy` (🚀 button on a product row) — takes an optional `platformOverride` (every grouping targets that platform) or, left blank, lets Claude recommend per-grouping. Reads Product Research + Product fields + campaign Research comprehensively. Also takes an optional `seedTitleId`: when a title (typically one sitting unassigned in the Strategies tab, no Growth Strategy of its own yet) seeds the run, that title's own Notes/keywords become the PRIMARY driver of the plan instead of Product Research/campaign Research, which become secondary supporting context — this is how an operator gives one specific title its own dedicated, separate Growth Strategy rather than attaching it to whatever already exists for that product (a product can hold several Growth Strategies at once, so this never overwrites anything). Writes properties (Strategy Name/Product/Campaign/Platform Override/Recommended Platforms/Status/Summary/Grouping Count) plus a structured page body (heading + rationale + title bullets + method/platform per grouping). **Never creates titles or touches a Method** — purely a reviewable recommendation; a product can have several over time (Status Draft/Approved/Archived), browsable via the row's "plans" dropdown (`listGrowthStrategies`/`getGrowthStrategy`, opens a read-only view modal).

### Strategy Slots and Planning-stage titles

Growth Strategy generation also bulk-creates `🎯 Strategy Slots` (`STRATEGY_SLOTS_DB`) — one lightweight row per planned title angle (e.g. "Behind the Scenes #1"), grouped under the Growth Strategy that produced them. A Slot is a placeholder, not a real title — visible/trackable in the microsite's **Strategies** tab with real fill-progress (`getCampaignGrowthStrategies`), not buried per-product. Filling one in (`generateTitleFromSlot`, the "+ title" button) writes ONE real Content Strategy title at **Status: Planning**, method-agnostic (no pillar content yet on purpose).

**Planning is deliberately excluded from the Development list.** A title stays in the Strategies tab (grouped under its Slot, or under "General"/"Unassigned Planning Titles" if it has no Slot) until its pillar gets written — the **✍️ Write Pillar** button there (`writeTitlePillar`) does both at once: writes the pillar and promotes Planning → Development in the same call, same as `generateTitleAssets`'s own pillar-write-on-first-run guarantee for Development-stage titles missing one. Only once a title reaches Development does it show there, grouped Stack → Product → Strategy (via the title's own `Growth Strategy` relation) → Title.

An unassigned Planning title (no Growth Strategy relation at all) can either 🔗 attach to an existing Growth Strategy for its product, or 🚀 generate a brand-new one seeded by its own content (`generateGrowthStrategy`'s `seedTitleId`, see above) — titles never generate a strategy implicitly, this is always an explicit operator action.

The separate `generateTitlesFromStrategy` action (reads a Method's own Strategy Brief, not a Slot — the "Generate Strategy" button in the Generate Assets modal) also creates titles at Status: Planning for the same reason, though these already carry real drafted content in their page body (not deferred like Slot-filled titles), so they typically just need a normal ⇄ status promotion once reviewed, not the Write Pillar step.
...
**Slot naming convention: `"{sequence} – {Post Type name}"`** (e.g. `"3 – Feature Benefit"`) — set at creation and kept in sync by `removeStrategySlot`/`insertStrategySlot` below (`renumberedSlotName` only rewrites names that already match the pattern; older/manual names are left alone).
